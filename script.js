...
class Player {
    constructor(name){
        this.name=name;this.level=1;this.exp=0;this.coins=0;
        this.inventory=new Inventory(20);this.equipment={};
        this.initGear();
    }
    initGear(){
    ...
    gainExp(n){
        this.exp+=n;
        if(this.exp>=this.level*100){this.exp-=this.level*100;this.level++;alert("Level Up! "+this.level);}
        $("level").textContent=this.level;$("exp").textContent=this.exp;
    }
    getStat(base){
        let t=1;Object.values(this.equipment).forEach(g=>{if(g&&g.stats[base])t*=g.stats[base];});return t;
    }
}
class Gear {
    constructor(name,rarity,stats){this.name=name;this.rarity=rarity;this.stats=stats;}
}
class Inventory {
    constructor(size){this.slots=Array(size).fill(null);}
    add(it){const i=this.slots.findIndex(s=>!s);if(i===-1)return false;this.slots[i]=it;return true;}
    remove(idx){const out=this.slots[idx];this.slots[idx]=null;return out;}
}
...
function renderGrid(){
    ...
    cont.innerHTML="";cont.style.gridTemplateColumns=`repeat(${GRID.c},1fr)`;
    for(let i=0;i<GRID.r;i++){
        for(let j=0;j<GRID.c;j++){
            ...
            }else if(GRID.flagged[i][j])cell.classList.add("flagged"),cell.textContent="🚩";
            cell.oncontextmenu=e=>{e.preventDefault();if(!GAME_OVER)toggleFlag(i,j);};
            cell.onmousedown=e=>{
                if(GAME_OVER)return;
                if(e.button===0){
...
function clickCell(i,j){
    ...
    if(FIRST_CLICK){GRID.placeMines(i,j);FIRST_CLICK=false;}
    GRID.reveal(i,j);
...
function lootDrop(){
    if(Math.random()<0.3){
        ...
        PLAYER.inventory.add(g);alert("Loot! "+g.name);
    }
}
function resetGame(){
...
function renderInv(){
    ...
    slots.innerHTML="";
    PLAYER.inventory.slots.forEach((it,idx)=>{
